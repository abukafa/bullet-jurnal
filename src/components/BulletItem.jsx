...
export default function BulletItem({
  ...
  const handleStatusToggle = async () => {
    ...
    setStatus(newStatus);
    await db.bullets.update(bullet.id, {
      status: newStatus,
      updatedAt: new Date(),
    });
  };
  ...
  const displayTime =
    bullet.time ||
    (bullet.createdAt
      ? `${String(new Date(bullet.createdAt).getHours()).padStart(2, "0")}:${String(new Date(bullet.createdAt).getMinutes()).padStart(2, "0")}`
      : null);
  ...
  return (
    ...
    >
      <div className="bullet-row">
        ...
        <div
          className={`bullet-text ${isComplete ? "strikethrough" : ""} ${compact ? "compact-view" : ""} ${searchResult ? "search-result-view" : ""} ${type === "note" ? (typeof bullet.pageId === "string" && bullet.pageId.startsWith("col_") ? "note-collection" : "note-daily") : ""}`}
          onClick={openDetail}
        ...
        >
          <div className="bullet-title-container">
            <div className="bullet-title">
              {bullet.text}
              {!compact && !searchResult && bullet.description && (
                <span className="bullet-has-notes mobile-only">...</span>
              )}
            </div>
          </div>
